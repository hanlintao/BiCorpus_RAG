	...
	else
	{
		...
		else
		{
			...
			foreach ($jsondata["body"]["tu"] as $tu)
			{
				...
				$content = $source_content." || ".$target_content;
				...
				if (isset($response['embedding'])) {
					...
					//var_dump($embedding);
					$insert_sql = "INSERT INTO `tmdata` (`source_content`, `target_content`,`embedding`,`source_lang`,`target_lang`,`file_id`) VALUES ('{$source_content}', '{$target_content}','{$embedding_json }','{$source}','{$target}','{$file_id}')";
